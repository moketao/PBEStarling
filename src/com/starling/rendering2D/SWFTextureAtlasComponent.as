package com.starling.rendering2D 
{
	...
	/**
	 * Dynamically creates a TextureAtlas from the loaded SWF file and movieClipClassName.
	 */
	public class SWFTextureAtlasComponent extends TextureAtlasComponent 
	{
		
		public var swf:SWFResource;
		
		public var movieClipClassName:String;
		...
		{
			if ( movieClipClassName == null )
				throw new Error("You must define the movieClipClassName that is exported in the SWF in order to create a texture atlas from it");
			...
			if ( swf != null )
			{
				if ( swf.isLoaded )
					...
					swf.addEventListener(ResourceEvent.LOADED_EVENT, onSwfLoaded );
			}
